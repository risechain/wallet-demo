...
export function useSessionKeys() {
  ...
  const sessionKeys: SessionKey[] = permissions?.map((permission: any, index: number) => ({
    ...
    hasPrivateKey: permission.key?.publicKey ? hasLocalPrivateKey(permission.key.publicKey) : false
  ...
  useEffect(() => {
    ...
    if (keyPair && sessionKeys.some(key =>
      ...
      try {
        ...
        if (keyData) {
          ...
          if (parsed.publicKey && parsed.privateKey) {
            ...
            const matchingSessionKey = sessionKeys.find(key =>
            ...
            if (matchingSessionKey) {
              bestKeyPair = { privateKey: parsed.privateKey, publicKey: parsed.publicKey }
    ...
    if (bestKeyPair && (!keyPair || keyPair.publicKey !== bestKeyPair.publicKey)) {
      keyPair = bestKeyPair
    }
  ...
  const createSessionKey = useCallback(async () => {
    ...
    try {
      ...
      localStorage.setItem(`porto.sessionKey.${publicKey}`, JSON.stringify({ privateKey, publicKey }))
  ...
  const executeWithSessionKey = useCallback(async (calls: Array<{ to: string; data?: string; value?: string }>) => {
    if (!keyPair) {
    ...
    const chainId = Hex.toNumber(await provider.request({ method: 'eth_chainId' }))
    ...
    try {
      ...
      if (keyToRevoke.hasPrivateKey) {
        ...
        if (typeof window !== 'undefined') {
          localStorage.removeItem(`porto.sessionKey.${keyToRevoke.publicKey}`)
        }
      }
